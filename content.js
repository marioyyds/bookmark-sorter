(function () {
  ...
  function makeOverlay(rect, color, outline) {
  ...
  function overlayRange(range, color) {
    const overlays = [];
  ...
  // ---- 页面命令系统 (Phase 1) ----
  // 结构化命令控制当前页面 DOM：高亮、滚动、改样式、点击、描边、读取。
  ...
  function executePageCommand(command, params) {
    params = params || {};
    switch (command) {
  ...
  */
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg && msg.type === 'kbPageCommand') {
      sendResponse(executePageCommand(msg.command, msg.params || {}));
      return false;
    }
    return false;
  });
