...
function fitPanelHeight() {
  ...
  const topGap = Math.max(12, Math.round(window.innerHeight * 0.10));
  const maxH = Math.max(320, window.innerHeight - topGap - 12);
  ...
  if (panel.classList.contains('docked') && !panel._manualPos) {
    panel.style.top = Math.max(12, Math.round(window.innerHeight * 0.10)) + 'px';
    panel.style.bottom = 'auto';
  }
}

// 初始停靠位置使用相对视口高度：大屏幕适当上移，小屏幕保持紧凑。
function getDockGap() {
...
function openPanel(x, y, docked) {
  ...
  // Cline 风格：审批入口属于输入区工具栏，始终紧贴输入框上方，不占据对话内容区。
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    if (dragState && panel) {
      ...
      if (leftSide || topSide) {
        ...
        if (topSide) panel.style.top = Math.max(0, bottom - h) + 'px';
