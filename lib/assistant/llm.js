...
async function callDeepSeekOnce(settings, messages, signal) {
...
export async function callDeepSeek(settings, messages, signal) {
  const MAX_RETRIES = 2;
  for (let attempt = 0; ; attempt++) {
    try {
      return await callDeepSeekOnce(settings, messages, signal);
    } catch (e) {
