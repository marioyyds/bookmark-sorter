...
const INTENT_DEFS = Object.freeze({
  [INTENTS.BROWSER]: {
    ...
    rules: [
      ...
      '优先 open_tab 打开搜索页 → get_page_snapshot 查看页面 → type_text/press_key 完成搜索 → page_command(click) 进入目标页面。',
      '点击结果链接若检测到新标签页会自动绑定，直接用 get_page_snapshot 查看新页面即可，不要先 list_tabs 再 switch_tab。',
      'page_command(click) 返回结果中会包含新页面的标题、URL 和元素摘要；直接据此判断目标是否已达成，不要再次调用 get_page_snapshot 重复验证。',
      '用户要求修改页面样式（标题变大、加粗、换色、高亮、描边）时，使用 page_command(set_style/highlight/outline)，以真实执行结果为准回复，不要凭空声称“已设置完成”。',
